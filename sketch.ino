...
http://192.168.xxx.xxx?getstill                 //Dapatkan gambar diam
http://192.168.xxx.xxx?framesize=size     //size= UXGA|SXGA|XGA|SVGA|VGA|CIF|QVGA|HQVGA|QQVGA Ubah resolusi gambar
http://192.168.xxx.xxx?quality=value    // value = 10 hingga 63
http://192.168.xxx.xxx?brightness=value    // value = -2 hingga 2
http://192.168.xxx.xxx?contrast=value    // value = -2 hingga 2 

Mencari IP Klien:
Cari IP: http://192.168.4.1/?ip
Reset jaringan: http://192.168.4.1/?resetwifi=ssid;kata sandi
*/
...
#include "soc/soc.h" //Digunakan untuk daya tidak stabil tidak reboot
#include "soc/rtc_cntl_reg.h" //Digunakan untuk daya tidak stabil tidak reboot

String Feedback=""; //Kembalikan pesan klien 

//Nilai parameter perintah
String Command="",cmd="",P1="",P2="",P3="",P4="",P5="",P6="",P7="",P8="",P9="";
//Nilai status instruksi
byte ReceiveState=0,cmdState=1,strState=1,questionstate=0,equalstate=0,semicolonstate=0;

// PERINGATAN!!! Pastikan Anda telah memilih Modul Wrover ESP32, 
// atau board lain yang PSRAM-nya aktif
...
void setup() {
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);  //Matikan catu daya dan hidupkan kembali ketika listrik tidak stabil.
  ...
  //ukuran bingkai drop-down untuk kecepatan bingkai awal yang lebih tinggi
  sensor_t * s = esp_camera_sensor_get();
  ...
  {
      delay(500);
      if ((StartTime+10000) < millis()) break;    //Tunggu 10 detik untuk terhubung
  } 

  if (WiFi.status() == WL_CONNECTED) {    //若連線成功
    WiFi.softAP((WiFi.localIP().toString()+"_"+(String)apssid).c_str(), appassword);   ////Atur SSID untuk menampilkan IP klien      
    Serial.println("");
    Serial.println("STAIP address: ");
    Serial.println(WiFi.localIP());  

    for (int i=0;i<5;i++) {  //Jika terhubung ke WIFI, flash akan berkedip cepat
      ledcWrite(4,10);
      delay(200);
      ledcWrite(4,0);
      delay(200);    
    ...
    for (int i=0;i<2;i++) {    //Jika WIFI tidak dapat terhubung, flash akan berkedip perlahan.
      ledcWrite(4,10);
      delay(1000);
      ledcWrite(4,0);
      delay(1000);    
...
//Antarmuka manajemen halaman beranda web yang disesuaikan
  ...
  <div id="result" style="width:320px;color:red"></div>
    ...
    function error_handle() {
      ...
      if (restartCount<=2) {
        ...
        result.innerHTML = "Get still error. <br>Silakan tutup halaman dan periksa ESP32-CAM";
    }  
  ...
  if (client) { 
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        getCommand(c);   //Dekomposisi karakter yang diperoleh dari buffer menjadi parameter perintah
                
        if (c == '\n') {
          if (currentLine.length() == 0) {    
            
            if (cmd=="getstill") {
              //FeedBack gambar berformat JPEG
              camera_fb_t * fb = NULL;
              ...
              //Kembalikan halaman beranda atau Feedback HTML
              client.println("HTTP/1.1 200 OK");
              client.println("Access-Control-Allow-Headers: Origin, X-Requested-With, Content-Type, Accept");
              client.println("Access-Control-Allow-Methods: GET,POST,PUT,DELETE,OPTIONS");
        ...
        if ((currentLine.indexOf("/?")!=-1)&&(currentLine.indexOf(" HTTP")!=-1)) {
          if (Command.indexOf("stop")!=-1) {  //Jika perintah berisi kata kunci stop, saluran akan segera terputus -> http://192.168.xxx.xxx/?cmd=aaa;bbb;ccc;stop
            client.println();
            client.println();
            client.stop();
          }
...
//Dekonstruksi string dan tempatkan ke dalam variabel
